
import './App.css';
import React from 'react';
import { API_KEY, API_URL_SERCH } from './Api';
import  BoockItem  from './components/BoockItem';
import 'antd/dist/antd.css';
import {  Layout, Menu, Breadcrumb } from 'antd';
import {  UserOutlined, LaptopOutlined, NotificationOutlined } from '@ant-design/icons';



class App extends React.Component {
  constructor() {
    super();
    this.state = {
      onSearch: 'science+fiction&download=epub',
      books: []

    }
  };
  getData = async function searchBook() {
    const response = await fetch(`${API_URL_SERCH}=${this.state.onSearch}&key=${API_KEY}`);
     const data = await  response.json();
     await this.setState({
         books: data.items
       })
   

  }
  componentDidMount() {
    this.getData();

  };
  componentDidUpdate(prevProps, prevState) {
    if (prevState.onSearch !== this.state.onSearch) this.getData();
  };

  render() {
    const onSearch = value => this.setState({ onSearch: value.toUpperCase() });
    const { SubMenu } = Menu;
    const { Header, Content, Footer, Sider } = Layout;

    return (
      <div className="App">
        <Layout>
          <Header className="header">
            <div className="logo" />
            <Menu theme="dark" mode="horizontal" defaultSelectedKeys={['2']}>
              <Menu.Item key="1">nav 1</Menu.Item>
              <Menu.Item key="2">nav 2</Menu.Item>
              <Menu.Item key="3">nav 3</Menu.Item>
            </Menu>
          </Header>
          <Content style={{ padding: '0 50px' }}>
            <Layout className="site-layout-background" style={{ padding: '24px 0' }}>
              <Sider className="site-layout-background" width={400}>
                <Menu
                  mode="inline"
                  defaultSelectedKeys={['1']}
                  defaultOpenKeys={['sub1']}
                  style={{ height: '100%' }}
                >
                  <SubMenu key="sub1" icon={<UserOutlined />} title="subnav 1">
                    <Menu.Item key="1">option1</Menu.Item>
                    <Menu.Item key="2">option2</Menu.Item>
                    <Menu.Item key="3">option3</Menu.Item>
                    <Menu.Item key="4">option4</Menu.Item>
                  </SubMenu>
                  <SubMenu key="sub2" icon={<LaptopOutlined />} title="subnav 2">
                    <Menu.Item key="5">option5</Menu.Item>
                    <Menu.Item key="6">option6</Menu.Item>
                    <Menu.Item key="7">option7</Menu.Item>
                    <Menu.Item key="8">option8</Menu.Item>
                  </SubMenu>
                  <SubMenu key="sub3" icon={<NotificationOutlined />} title="subnav 3">
                    <Menu.Item key="9">option9</Menu.Item>
                    <Menu.Item key="10">option10</Menu.Item>
                    <Menu.Item key="11">option11</Menu.Item>
                    <Menu.Item key="12">option12</Menu.Item>
                  </SubMenu>
                </Menu>
              </Sider>
              <Content style={{ padding: '0 24px', minHeight: "100vh"}}>
                <BoockItem data={this.state.books}/>

              </Content>
            </Layout>
          </Content>
          <Footer style={{ textAlign: 'center' }}>Ant Design ©2018 Created by Ant UED</Footer>
        </Layout>
      </div>
    );
  }
}
export default App;
